class Solution {
    ...
    public List<List<Integer>> kSmallestPairs(int[] nums1, int[] nums2, int k) {
        ...
        while(k>0 ){
            ...
            if(p.i+1<n1 && !set.contains((p.i+1)+" "+p.j)) {
                pq.add(new Pair(nums1[p.i+1]+nums2[p.j],p.i+1,p.j));
                set.add((p.i+1)+" "+p.j);
            }
            //System.out.println((p.i+1)+""+p.j);
            if(p.j+1<n2&& !set.contains(p.i+" "+(p.j+1))){
                pq.add(new Pair(nums1[p.i]+nums2[p.j+1],p.i,p.j+1));
                set.add(p.i+" "+(p.j+1));
            } 
